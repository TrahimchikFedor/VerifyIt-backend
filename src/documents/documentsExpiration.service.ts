...
export class DocumentExpirationService {
  ...
  async checkExpiredDocuments() {
    try {
      ...
      this.logger.error(`Ошибка при проверке истекших документов: ${error.message}`, this.name);
    }
  }
  ...
  async checkExpiringSoon() {
    try {
      ...
      this.logger.log(`Обработано ${expiringSoon.length} документов, истекающих скоро`, this.name);
    } catch (error) {
      this.logger.error(`Ошибка при проверке истекающих документов: ${error.message}`, this.name);
    }
  }
}